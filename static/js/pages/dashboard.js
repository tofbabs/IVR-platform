$(function () {
    function t(t, e) {
        ...
        var n = d3.select(t), r = {
            ...
            bottom: 40,
            left: 50
        }, i = n.node().getBoundingClientRect().width - r.left - r.right, e = e - r.top - r.bottom, s = 30, l = n.append("div").attr("class", "d3-tip e").style("display", "none"), o = d3.time.format("%m/%d/%y %H:%M"), d = d3.time.format("%H:%M"), c = ["#03A9F4", "#29B6F6", "#4FC3F7", "#81D4FA", "#B3E5FC", "#E1F5FE"], u = d3.time.scale().range([0, i]), p = d3.scale.linear().range([e, 0]), f = d3.scale.ordinal().range(c), h = d3.svg.axis().scale(u).orient("bottom").ticks(d3.time.hours, 4).innerTickSize(4).tickPadding(8).tickFormat(d3.time.format("%H:%M")), m = d3.svg.axis().scale(p).ticks(6).innerTickSize(4).outerTickSize(0).tickPadding(8).tickFormat(function (t) {
            return t / 1e3 + "k"
        }), g = m, y = d3.svg.axis().scale(p).orient("left").ticks(6).tickPadding(8).tickFormat("").tickSize(-i, 0, 0), v = n.append("svg"), x = v.attr("width", i + r.left + r.right).attr("height", e + r.top + r.bottom).append("g").attr("transform", "translate(" + r.left + "," + r.top + ")"), b = d3.layout.stack().offset("silhouette").values(function (t) {
            return t.values
        }).x(function (t) {
            return t.date
        }).y(function (t) {
        ...
        d3.csv("../../static/demo_data/dashboard/traffic_sources.csv", function (a, c) {
    ...
    function e(t, e) {
        ...
        function n() {
            ...
            var h = c.domain().map(function (t) {
                ...
                return y(t.value)
            }).interpolate("cardinal"), g = d3.time.scale().domain([d3.min(h, function (t) {
            ...
            })]).range([0, s]), y = d3.scale.linear().domain([d3.min(h, function (t) {
                return d3.min(t.values, function (t) {
                    return t.value
                })
            }), d3.max(h, function (t) {
            ...
            var b = p.selectAll(".lines").data(h), w = b.enter().append("g").attr("class", "lines").attr("id", function (t) {
                return t.name + "-line"
            });
    ...
    function n(t, e, a) {
        var n = d3.select(t), r = {
            ...
            return f(t.value)
        }).interpolate("monotone"), p = d3.time.scale().range([0, i]), f = d3.scale.linear().range([e, 0]), h = d3.svg.axis().scale(p).orient("bottom").ticks(d3.time.days, 6).innerTickSize(4).tickPadding(8).tickFormat(d3.time.format("%b %d"));
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function (t, m) {
            function g() {
                var t = d3.mouse(this), a = t[0], i = (t[1], p.invert(a)), s = l(m, i), d = m[s - 1], c = m[s], u = i - d.date > c.date - i ? c : d;
    ...
    function i(t, e, a, n, r, i, s, l) {
        ...
        var m = d3.scale.linear().range([0, p]), g = d3.scale.linear().range([n - 5, 5]);
    ...
    function s(t, e) {
        ...
        var f = d3.time.scale().range([l, s - l]), h = d3.scale.linear().range([e, 5]);
    ...
    function u(t, e, a, n, r, i, s, l, o) {
        ...
        var h = d3.select(t), m = h.node().getBoundingClientRect().width, g = d3.scale.ordinal().rangeBands([0, m], .3), y = d3.scale.linear().range([0, a]);
        g.domain(d3.range(0, p.length)), y.domain([0, d3.max(p)]);
        var v = h.append("svg"), x = v.attr("width", m).attr("height", a).append("g"), b = x.selectAll("rect").data(p).enter().append("rect").attr("class", "d3-random-bars").attr("width", g.rangeBand()).attr("x", function (t, e) {
            return g(e)
    ...
    function p(t, e, a, n, r, i, s, l) {
        ...
        var d = d3.select(t), c = 0, u = 32, p = r, f = 2 * Math.PI, h = d3.format(".0%"), m = 2 * e, g = Math.abs((p - c) / .01), y = c > p ? -.01 : .01, v = d.append("svg"), x = v.attr("width", m).attr("height", m).append("g").attr("transform", "translate(" + m / 2 + "," + m / 2 + ")"), b = d3.svg.arc().startAngle(0).innerRadius(e).outerRadius(e - a);
            ...
            d3.bullet = function () {
                function i(t) {
                    t.each(function (t, e) {
                        ...
                        var C = m || v.tickFormat(8), M = y.selectAll(".bullet-tick").data(v.ticks(8), function (t) {
                ...
                var o = "left", d = !1, c = 750, u = t, p = e, f = a, h = 30, m = null;
